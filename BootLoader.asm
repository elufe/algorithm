...
.SCREENCLEARLOOP:                   ; 화면을 지우는 루프
    ...
    mov ah, 0x04                        ; BIOS 서비스 번호 2(Read Sector)
...
;	pop si
    ...
    push MESSAGE2               ; 출력할 메시지의 어드레스를 스택에 삽입
    push 1                      ; 화면 Y 좌표(0)를 스택에 삽입
    push 0                      ; 화면 X 좌표(0)를 스택에 삽입
    call PRINTMESSAGE           ; PRINTMESSAGE 함수 호출
    add  sp, 6                  ; 삽입한 파라미터 제거
...
MESSAGE2:    db "aa/aa/aaaa", 0          						; 마지막은 0으로 설정하여 .MESSAGELOOP에서 
;MESSAGE2:    db si, 0          						; 마지막은 0으로 설정하여 .MESSAGELOOP에서 
